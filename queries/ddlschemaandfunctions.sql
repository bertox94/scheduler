...
CREATE OR REPLACE FUNCTION public.getFirstId()
    RETURNS INTEGER AS
$$
DECLARE
    missing_id INTEGER;
...
CREATE OR REPLACE FUNCTION public.generateOrderOccurrences(
    end_preview INT8,
    id_user INT8
)
    RETURNS VOID AS
    ...
    FOR rec IN SELECT * FROM repeatedorder
        LOOP
            ...
            WHILE curr_date <= end_prev and (rec.rlim = 'to a specific date' and curr_date <= end_date or
                                             rec.rlim = 'for a number of times' and iter <= rec.repeatingtimes or
                                             rec.rlim = 'unlimited')
                LOOP
